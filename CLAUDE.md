...
- **Spacing is cell-level, not per-word** — `spacing.ts` weights the existing random pickers by how due a paradigm × case × number cell is; there is no due queue and no per-lemma SM-2. Lifetime `caseScores`/`paradigmScores`/`lemmaScores` are for stats, dashboards and the lemma nudge, not scheduling. Every write of `cell_schedule` (sync endpoint, login merge, first upload) goes through the `merge_cell_schedule` RPC (migration 039), which merges per cell (latest attempt wins) under a row lock so concurrent tabs/devices can't overwrite each other; never write the column wholesale except the profile reset. A client that omits the field leaves the column alone.
