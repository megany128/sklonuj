#!/bin/bash
# qa2-signin.sh <cdp_port> <email> [password]
PORT=$1
EMAIL=$2
PW=${3:-'QA2Test!234'}
...
sleep 1
# Switch to login mode if currently in signup
AB eval "(() => { const btn=Array.from(document.querySelectorAll('button')).find(b=>b.textContent.includes('Already have')); if(btn){btn.click();return 'switched';} return 'already-login'; })()" >/dev/null
sleep 1
