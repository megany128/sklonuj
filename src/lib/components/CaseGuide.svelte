<script lang="ts">
	...
	const cases: CaseInfo[] = [
		...
		},
		{
			...
			darkColor: 'dark:bg-sky-900 dark:text-sky-300'
		}
	];
</script>
...
<div class="w-full">
	...
	>
		<div
			class="mt-2 space-y-2 rounded-2xl border border-slate-200/80 bg-white p-4 dark:border-slate-700/60 dark:bg-slate-800/80"
		>
			{#each cases as c (c.name)}
				<div class="rounded-xl border border-slate-100 p-3 dark:border-slate-700/40">
					<div class="flex items-start gap-2.5">
						<span
							class="mt-0.5 shrink-0 rounded-full px-2 py-0.5 text-[0.65rem] font-bold {c.color} {c.darkColor}"
						>
							{c.name}
						</span>
						<div class="min-w-0">
