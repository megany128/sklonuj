...
{#if result && !result.correct && (whyNote || templateWhy)}
	<div
		...
		class="w-full rounded-[24px] border border-card-stroke bg-card-bg p-5"
	>
		<div class="mb-3 flex items-center gap-2">
			<div class="flex h-6 w-6 items-center justify-center rounded-full bg-shaded-background">
				<svg
			...
			<p class="text-sm font-semibold text-text-default">Why?</p>
		</div>

		{#if templateWhy}
			<p class="text-sm leading-relaxed text-darker-subtitle">
				{templateWhy}
			</p>
		{/if}

		{#if whyNote}
			<p class="text-sm leading-relaxed {templateWhy ? 'mt-2' : ''} text-text-subtitle">
				{whyNote}
			</p>
		{/if}
	</div>
