...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	// Wave 1: everything keyed only on the user — active teacher classes, student
	// memberships, and archived classes (always checked, even with no active classes).
	const [teacherResult, studentResult, archivedResult] = await Promise.all([
		supabase
			.from('classes')
	...
	if (Array.isArray(archivedResult.data)) {
		for (const row of archivedResult.data) {
			if (isClassRow(row)) {
	...
	// Wave 2: per-class fan-out for both roles at once — the teacher's rosters and
	// assignment counts (keyed on teacher class ids) and the student's assignments
	// (keyed on student class ids) are independent of each other.
	const teacherClassIds = teacherClasses.map((c) => c.id);
	const membershipCountsPromise =
		teacherClassIds.length > 0
			...
			: null;
	const assignmentCountsPromise =
		teacherClassIds.length > 0
			? supabase.from('assignments').select('id, class_id').in('class_id', teacherClassIds)
			: null;

	const studentClassIds = studentClasses.map((c) => c.classId);
	const studentAssignmentsPromise =
	...
	const [membershipCountsResult, assignmentCountsResult, studentAssignmentsResult] =
		await Promise.all([
			membershipCountsPromise,
			assignmentCountsPromise,
	...
	const countData = membershipCountsResult?.data;
	...
	const assignmentCountData = assignmentCountsResult?.data;
	...
	const assignmentData = studentAssignmentsResult?.data;
	...
	// Wave 3: user_progress for every student across the teacher's classes (admin client —
	// RLS prevents teachers from reading it) and the student's own assignment progress.
	// Again independent of each other.
	const supabaseUrl = env.PUBLIC_SUPABASE_URL;
	const serviceRoleKey = privateEnv.SUPABASE_SERVICE_ROLE_KEY;
	const adminClient =
		allStudentIds.size > 0 && supabaseUrl && serviceRoleKey
			? createClient(supabaseUrl, serviceRoleKey)
			: null;
	const userProgressPromise = adminClient
		? adminClient
				.from('user_progress')
	...
	const [userProgressResult, studentProgressResult] = await Promise.all([
		userProgressPromise,
	...
	// For teacher classes, attach student counts, assignment counts, and accuracy stats
	...
	// Student's own progress for the assignments above
	const studentProgressList: AssignmentProgressRow[] = [];
	const progressData = studentProgressResult?.data;
